...
ll solve(int i , int he)
{
    ...
    if(~rst)
        return rst;
    ll ch1 = 1e17 , ch2 = 1e17;
    if(x[i] == '1')
    {
        if(he == 1)
        ch1 = 1e17;
        else
        ch1 = 2 * pill+ road + solve(i+1 , 2);
    }
    else
    {
        if(he == 1)
        {
            ch1 = solve(i+1,1) + pill + road;
            ch2 = solve(i+1 , 2) + 2 * (pill + road);
        }
        else
        {
            ch1 = solve(i+1,2) + 2 * pill + road;
            ch2 = solve(i+1 , 1) + pill + 2 * road;
        }
    }
