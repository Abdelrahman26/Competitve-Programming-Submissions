...
int main()
{
    ...
    ll n;
    cin >> n;
    ll ncn_2 = fact(n)/(fact(n/2)*fact(n/2));/// all possible combinations
    ll one_fixed = fact(n/2 - 1); /// all possible combinations in one of two set when first element in any set is fixed
    cout << ncn_2 * one_fixed * one_fixed / 2<<endl;/// divided by 2 to avoid [(1,2) , (3,4)] and [(3,4) , (1,2)]
}
